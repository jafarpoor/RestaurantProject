...
{
  public  class GetListAllOrderService : IGetListAllOrderService
    {

        private readonly IDatabaseContext _dataBaseContext;

        public GetListAllOrderService(IDatabaseContext dataBaseContext)
        {
            _dataBaseContext = dataBaseContext;
        }

        public ResultDataModel<List<GetListAllOrderDataModel>> GetList(string OrderStatus)
        {
           IQueryable<Payment> Result = _dataBaseContext.Payments
            ...
            if (Result == null)
                return new ResultDataModel<List<GetListAllOrderDataModel>>
                {
                    Data = null ,
                    IsSuccess = false ,
          ...
          var model =  Result.Select(p => new GetListAllOrderDataModel
                         {
                             OrderId = p.Order.Id,
                             PayAmount = p.Amount,
